...
private[protocol] trait StateMetadata extends Serializable {
  ...
  sealed trait Metadata {
     ...
     *
     * In practice, this will be `self` if __not__ running in clustered mode, and the [[com.teambytes.inflatable.raft.cluster.ClusterRaftActor]]
    ...
    def membersExceptSelf(slf: ActorRef) = config.members.filterNot { mem =>
      mem == clusterSelf
    }// || mem == slf }
