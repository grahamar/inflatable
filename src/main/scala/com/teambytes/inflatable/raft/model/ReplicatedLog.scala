...
/**
 * @param defaultBatchSize number of commands that can be sent together in one [[com.teambytes.inflatable.raft.protocol.RaftProtocol.AppendEntries]] message.
 */
private[inflatable] case class ReplicatedLog[Command](
...
private[inflatable] class EmptyReplicatedLog[T](defaultBatchSize: Int) extends ReplicatedLog[T](List.empty, -1, defaultBatchSize) {
  override def lastTerm = Term(0)
  override def lastIndex = 0
}

private[inflatable] object ReplicatedLog {
  def empty[T](defaultBatchSize: Int): ReplicatedLog[T] = new EmptyReplicatedLog[T](defaultBatchSize)
}

private[inflatable] case class Entry[T](
  command: T,
  term: Term,
  index: Int,
...
private[inflatable] trait SnapshotEntry {
  this: Entry[_] =>
