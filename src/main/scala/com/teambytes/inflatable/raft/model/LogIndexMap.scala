...
import com.teambytes.inflatable.raft.{StableClusterConfiguration, JointConsensusClusterConfiguration, ClusterConfiguration}
 ...
 *
 */
private[inflatable] case class LogIndexMap private (private var backing: Map[ActorRef, Int], private val initializeWith: Int) {
  ...
  def consensusForIndex(config: ClusterConfiguration): Int = config match {
    case StableClusterConfiguration(_, members, _) =>
      indexOnMajority(members)

    case JointConsensusClusterConfiguration(_, oldMembers, newMembers, _) =>
...
private[inflatable]object LogIndexMap {
