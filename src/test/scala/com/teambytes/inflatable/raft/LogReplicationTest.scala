...
class LogReplicationTest extends RaftSpec(callingThreadDispatcher = false)
  ...
  it should "replicate the missing entries to Follower which was down for a while" in {
    ...
    initialLeader ! ChangeConfiguration(ClusterConfiguration(isLocal = false, members().toSet -- failingMembers)) // 4, 5
    ...
    allMembers foreach { _ ! ChangeConfiguration(ClusterConfiguration(isLocal = false, allMembers)) } // 8, 9
