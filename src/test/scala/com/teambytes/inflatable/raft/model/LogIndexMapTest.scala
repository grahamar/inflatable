...
import com.teambytes.inflatable.raft.{ClusterConfiguration, RaftSpec}
...
class LogIndexMapTest extends RaftSpec with BeforeAndAfter {
  ...
  "consensus" should "be achieved on the majority of nodes, if in stable configuration state" in {
    // given
    val stableConfig = ClusterConfiguration(isLocal = false, probes)
  ...
  it should "be achieved on the both majorities in case of an JointConsensus configuration" in {
    ...
    val initialConfig = ClusterConfiguration(isLocal = false, probes)
    val nextConfig = ClusterConfiguration(isLocal = false, probes + probe4.ref + probe5.ref)
